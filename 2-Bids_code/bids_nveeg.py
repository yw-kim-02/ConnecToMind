...
# 원본 데이터와 출력 폴더 경로
source_dir = "/research/06-Neural_decoding/0-rawdata/Naturalistic viewing EEG"
output_dir = "/research/06-Neural_decoding/1-BIDS"
...
for i, sub in tqdm(enumerate(subjects,start=1), desc=f"Processing subject"):
    new_sub = f"sub-{i+20:02d}" # BIDS의 이름은 sub-<index>로 해야함
    sub_dir = os.path.join(output_dir, new_sub)  # BIDS 폴더: sub
    os.makedirs(sub_dir, exist_ok=True)

    subject_path = os.path.join(source_dir, sub)  # Raw 폴더: sub
    sessions = [d for d in os.listdir(subject_path) if d.startswith("ses-") and d[-2:].isdigit()] # Raw 폴더 ses모음
    ...
    for i, ses in tqdm(enumerate(sessions,start=1), desc=f"Processing {new_sub}"):
        ...
        source_anat_nii_path = os.path.join(session_path, "anat", f"{sub}_{ses}_T1w.nii.gz") # Raw 폴더: sub-ses-anat
        source_anat_json_path = os.path.join(session_path, "anat", f"{sub}_{ses}_T1w.json") # Raw 폴더: sub-ses-anat
        ...
        if os.path.exists(source_anat_nii_path):
            ...
            subprocess.run(["rsync", "-avz", "--ignore-existing", source_anat_nii_path, target_anat_nii_path])
        ...
        if os.path.exists(source_anat_json_path):
            ...
            subprocess.run(["rsync", "-avz", "--ignore-existing", source_anat_nii_path, target_anat_json_path])
        ...
        if os.path.exists(func_path):
            for func_file in tqdm(os.listdir(func_path), desc=f"Processing {new_sub} ses-{i:02d} func"):
                if "events" in func_file or "bold" in func_file:
                    ...
                    func_target = os.path.join(func_dir, func_file.replace(sub, new_sub).replace(f"{ses}", f"ses-{i:02d}"))
                    subprocess.run(["rsync", "-avz", "--ignore-existing", func_source, func_target])