...
# 원본 데이터와 출력 폴더 경로
source_dir = "/nas-tmp/research/03-Neural_decoding/1-raw_data/bold5000"
...
for i, sub in tqdm(enumerate(subjects,start=1), desc=f"Processing subject"):
    new_sub = f"sub-{i+8:02d}" # BIDS의 이름은 sub-<index>로 해야함
    ...
    for i, ses in tqdm(enumerate(sessions,start=1), desc=f"Processing {new_sub}"):
        ...
        source_anat_nii_path = os.path.join(subject_path, sessions[-1], "anat", f"{sub}_{sessions[-1]}_T1w.nii.gz") # Raw 폴더: sub-ses-anat
        source_anat_json_path = os.path.join(subject_path, sessions[-1], "anat", f"{sub}_{sessions[-1]}_T1w.json") # Raw 폴더
        ...
        if os.path.exists(func_path):
            for func_file in tqdm(os.listdir(func_path), desc=f"Processing {new_sub} ses-{i:02d} func"):
                if "events" in func_file or "bold" in func_file:
                    ...
                    func_target = os.path.join(func_dir, func_file.replace(sub, new_sub).replace(f"{ses}", f"ses-{i:02d}").replace("task-5000scenes", "task-image"))
